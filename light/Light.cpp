...
void Light::applyNotificationState(const LightState& state) {
    ...
    if (state.flashMode == Flash::TIMED && state.flashOnMs > 0 && state.flashOffMs > 0) {
        ...
        WriteToFile(BLUE_ATTR(lut_pattern), GetScaledDutyPcts(white_brightness));
        ...
        WriteToFile(GREEN_ATTR(lut_pattern), GetScaledDutyPcts(white_brightness));
        WriteToFile(GREEN_ATTR(step_ms), static_cast<uint32_t>(step_duration));
        WriteToFile(GREEN_ATTR(breath), 1);

        // White
        WriteToFile(WHITE_ATTR(lo_idx), 0);
        WriteToFile(WHITE_ATTR(delay_off), static_cast<uint32_t>(pause_hi));
        WriteToFile(WHITE_ATTR(delay_on), static_cast<uint32_t>(state.flashOffMs));
        WriteToFile(WHITE_ATTR(lut_pattern), GetScaledDutyPcts(white_brightness));
        WriteToFile(WHITE_ATTR(step_ms), static_cast<uint32_t>(step_duration));
        WriteToFile(WHITE_ATTR(breath), 1);
    } else {
        WriteToFile(BLUE_ATTR(brightness), white_brightness);
        WriteToFile(GREEN_ATTR(brightness), white_brightness);
