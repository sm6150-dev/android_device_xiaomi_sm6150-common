...
int main() {
    ...
    sp<IDisplayModes> dm = new DisplayModes();
    sp<IPictureAdjustment> pa = new PictureAdjustment(controller);
    ...
    if (status != OK) {
        LOG(ERROR) << "Could not register service for LiveDisplay HAL DisplayMode Iface ("
                   << status << ")";
        return 1;
    }
