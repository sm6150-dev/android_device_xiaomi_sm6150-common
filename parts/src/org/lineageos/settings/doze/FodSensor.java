...
class FodSensor {
    ...
    private static final String TAG = "FodSensor";
    public FodSensorListener mFodSensorListener;
    private final SensorManager mSensorManager;

    interface FodSensorListener {
        void onDeviceMove();
    ...
    private final SensorEventListener mNonUIListener = new SensorEventListener() {
        ...
        public void onSensorChanged(SensorEvent sensorEvent) {
            if (sensorEvent != null && mFodSensorListener != null) {
                float[] fArr = sensorEvent.values;
                if (fArr[0] == 0.0f) {
                    mFodSensorListener.onExitNonUI((int) sensorEvent.values[0]);
                } else if (fArr[0] == 1.0f || fArr[0] == 2.0f) {
                    mFodSensorListener.onEnterNonUI((int) sensorEvent.values[0]);
                } else {
    ...
    private final SensorEventListener mFodDetectorSensorListener = new SensorEventListener() {
        ...
        public void onSensorChanged(SensorEvent sensorEvent) {
            if (sensorEvent != null && mFodSensorListener != null) {
                float[] fArr = sensorEvent.values;
                if (fArr[0] == 1.0f) {
                    mFodSensorListener.onDeviceMove();
                } else if (fArr[0] == 2.0f) {
                    mFodSensorListener.onDeviceStable();
                } else if (fArr[0] == 3.0f) {
                    mFodSensorListener.onDeviceFodDetector();
                } else {
                    Log.w(TAG, "event.values[0] = " + sensorEvent.values[0]);
                }
            }
